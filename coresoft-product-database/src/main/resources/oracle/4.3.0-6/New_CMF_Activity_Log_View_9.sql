...
SELECT DISTINCT DS.SUBCRIPTION_ID,
  CASE
    WHEN (DA.IS_PRIMARY !=1)
    THEN DA.ACCOUNT_NUMBER
  END AS LINKED_ACCOUNT,
  CASE
    WHEN (DAU.REV_TYPE =2)
    THEN DAU.ACCOUNT_NUMBER
  END AS DELETE_ACCOUNT,
...
OR TRUNC(DA.UPDATED_DATE) BETWEEN TRUNC(SYSDATE-1) AND TRUNC(SYSDATE)
OR TRUNC(DAU.UPDATED_DATE) BETWEEN TRUNC(SYSDATE-1) AND TRUNC(SYSDATE));